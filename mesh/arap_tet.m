function [U,data,SS,R] = arap_tet(V,F,Adjinfo,demoldDir,plane,  sigma,b,bc,varargin)
...
%             Tet = U(F(J(i),:),:)*R(:,:,J(i));
%             Tet(K(i),:)=[];            
            point1   = U(boundaryF(i,1),:);                    
            dist1    = distancePointPlane(point1, plane);
            point2   = U(boundaryF(i,2),:);                    
            dist2    = distancePointPlane(point2, plane);
            point3   = U(boundaryF(i,3),:);                    
            dist3    = distancePointPlane(point3, plane);

            normalbf = cross(U(boundaryF(i,2),:)-U(boundaryF(i,1),:),U(boundaryF(i,3),:)-U(boundaryF(i,1),:));
            ...
            if dot(demoldDir,normalbf) < sigma && dist1 > 0 && dist2 > 0 && dist3 > 0                
                ...
                %%
                q(J(i),:) = quatmultiply(q(J(i),:),axisangle2quat(axTemp,-theta));
            end
